...
class GuideDashboardRepoImpl implements GuideDashboardRepo {
  ...
  Future<Either<Failure, GuideStatisticsModel>> getStatistics() async {
    try {
      final response =
          await apiConsumer.get(EndPoint.guideDashboardStatistics);
  ...
  Future<Either<Failure, List<TourPerformanceModel>>>
    ...
    try {
      final response =
          await apiConsumer.get(EndPoint.guideDashboardToursPerformance);
      final list = (response as List)
          .map(
              (e) => TourPerformanceModel.fromJson(e as Map<String, dynamic>))
          .toList();
      return Right(list);
    } on ServerException catch (e) {
      return Left(ServerFailure(e.errModel.errorMessage));
  ...
  Future<Either<Failure, List<WalletTransactionModel>>>
    ...
    try {
      final response =
          await apiConsumer.get(EndPoint.guideDashboardWalletTransactions);
      final list = (response as List)
          .map((e) =>
  ...
  Future<Either<Failure, List<RecentActivityModel>>> getActivities() async {
    try {
      final response =
          await apiConsumer.get(EndPoint.guideDashboardActivities);
      final list = (response as List)
  ...
  Future<Either<Failure, List<GuideTourSummaryModel>>> getMyTours() async {
    try {
      ...
      final list = (response as List)
          .map((e) =>
              GuideTourSummaryModel.fromJson(e as Map<String, dynamic>))
          .toList();
      return Right(list);
    } on ServerException catch (e) {
      return Left(ServerFailure(e.errModel.errorMessage));
  ...
  Future<Either<Failure, GuideTourDetailModel>> getTourDetails({
    ...
    try {
      final response =
          await apiConsumer.get(EndPoint.guideDashboardTour(id: id));
  ...
  Future<Either<Failure, String>> deleteTour({required String id}) async {
    try {
      final response =
          await apiConsumer.delete(EndPoint.guideDashboardTour(id: id));
      final message =
          (response is Map) ? response['message'] as String? ?? '' : '';
